...
//              contentType: false, // tell jquery not to set contentType
                success: function(data) {
					...
					var html_val_='<option>选择老师</option>'
					for (var i = 0; i < data.res.data.length; i++) {  
						html_val_=html_val_+'<option value="'+data.res.data[i].id+'">'+data.res.data[i].name+'</option>';
					}
					$("#uid").html(html_val_);
                }
            });
   ...
   $.ajax({
            url: urlip +"title/select",
            type:'POST',
            data: {},
    		dataType: 'JSON',
    		success: function(data) {
				console.log(data); 
				var html_val_='<option>选择毕设题目</option>'
				for (var i = 0; i < data.res.data.length; i++) {  
					html_val_=html_val_+'<option value="'+data.res.data[i].paper_headline+'">'+data.res.data[i].paper_headline+'</option>';
				}
				$("#headline").html(html_val_);
            }
        });  
    ...
    function dormitory_search(name,id){ 
        ...
        success: function (data) { 
         	...
         	if(data.code==0){
         		for (var i = 0; i < data.res.data.length; i++) { 
         			var remark = data.res.data[i].remark.substring(0,10)+"...";
         				
         				counts = counts +   " <tr>\n" + 
				                	"                                <td hidden id='"+data.res.data[i].id+"'>"+data.res.data[i].id+"</td>\n" +  
								    "                                <td >"+data.res.data[i].student_uid__name+"</td>\n" +
								    "								 <td >"+data.res.data[i].student_uid__username+"</td>\n" + 
								    "                                <td >"+data.res.data[i].production_name+"</td>\n" + 
								    "                                <td >"+data.res.data[i].production_url+"</td>\n" + 
								    "                                <td >"+data.res.data[i].headline+"</td>\n" + 
								    
								    "                                <td >"+data.res.data[i].mark+"</td>\n" + 
								    
								    "                                <td style='width: 200px;' rid='"+data.res.data[i].remark+"'>"+remark+"</td>\n" + 
								    "                                <td >"+timestampToTime2(data.res.data[i].add_time)+"</td>\n" +
								    "                                <td >"+timestampToTime2(data.res.data[i].update_time)+"</td>\n" +
                                    "                                <td style='width: 300px;'>\n" +
                                    "                                    <a href=\"http://127.0.0.1:800/video/download_pinyu?id="+data.res.data[i].id+"&name="+data.res.data[i].production_name+"\" style=\"color:green;\" class=\"btn btn-info btn-lg bofangbtn\" >\n" +
                                    "                                        <span class=\"glyphicon glyphicon-play-circle \"></span> 下载评语\n" +
                                    "                                    </a>"+
...
//                                  "                                    </a>\n" +
	...
	// 打开编辑用户信息
	    ...
	    $('#edit_value').click(function(){
	        ...
	        var edit_mark=$('input[name="edit_mark"]').val();
	        var edit_remark=$('input[name="edit_remark"]').val();
	        $.ajax({
	            url: urlip+"video/modify",
	            type: "post",
	            dataType:"JSON",
	            xhrFields: {
			   		withCredentials: true
	        ...
	        edit_pkid="#"+edit_pkid;
        	$(edit_pkid).parent().children().eq(6).html(edit_mark); 
        	$(edit_pkid).parent().children().eq(7).html(edit_remark); 
