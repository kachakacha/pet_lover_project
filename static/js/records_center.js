...
//	 $(".style_color1").hide()  
    ...
    $.ajax({
     ...
     $.ajax({
        		...
        		success: function(data) {
					...
					for (var i = 0; i < data.res.data.length; i++) {  
						html_val_=html_val_+'<option value="'+data.res.data[i].headline+'">'+data.res.data[i].headline+'</option>';
					}
	...
	// 打开编辑用户信息
	    $(document).on('click', '.theme-compile', function() { 
			...
			var tr7=$(this).parent().parent().children().eq(7).attr("rid");
			 console.log(tr7)
			$("#edit_pkid").val(tr0);
			$("#edit_name").val(tr1);
	        $("#edit_mark").val(tr6); 
